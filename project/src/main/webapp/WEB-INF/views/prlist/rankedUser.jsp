				...
				<div class="container-fluid">
					...
					<div class="row">
						<c:forEach var="product" items="${rankView}">
							<div class="col-xl-3 col-md-6 mb-4 border">
								...
								<a
									href="/pr?large=${product.CATEGORY_LARGE}&medium=${product.CATEGORY_MEDIUM}&small=${product.CATEGORY_SMALL}&sub_category=${product.CATEGORY_SUB_CATEGORY}&num=${product.MARKET_NUM}">
								...
								<div class="btn-group" role="group" aria-label="Button group">
									<form class="actionForm" action="/market/register">
										...
										<button hidden="hidden"
											class="participateButton btn-primary btn-needlogin"
					...
					<div class="pagination">
						<c:if test="${pageMaker.prev}">
							<a
								href="${pageContext.request.contextPath}/prlist/rankedUser?page=${pageMaker.startPage - 1}">&laquo;
								이전</a>
						</c:if>
						...
						<c:forEach begin="${pageMaker.startPage}"
							...
							<c:choose>
								...
								<c:otherwise>
									<a
										href="${pageContext.request.contextPath}/prlist/rankedUser?page=${pageNum}">${pageNum}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${pageMaker.next}">
							<a
								href="${pageContext.request.contextPath}/prlist/rankedUser?page=${pageMaker.endPage + 1}">다음
								&raquo;</a>
						</c:if>
					</div>
